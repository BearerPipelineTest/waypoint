...
func RunnerConfig(
	...
	case *pb.Token_Runner_:
		...
		runnerId, err := s.DecodeId(k.Runner.Id)
		if err != nil {
			log.Error("Failed to decode id from runner config request token", "id", k.Runner.Id, "error", err)
			return status.Errorf(codes.Internal, "invalid id")
		}
		...
		if runnerId != "" && !strings.EqualFold(runnerId, record.Id) {
